...
export default function ContactPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setSubmitting(true);
        const toastId = toast.loading("Göndərilir...");

        try {
            const res = await submitRegistration({ ...result.data, serviceTitle: "Əlaqə Formu" });
            if (res.success) {
                toast.success("Müraciətiniz göndərildi! Tezliklə sizinlə əlaqə saxlayacağıq.", { id: toastId });
                setFormData({ name: "", surname: "", phone: "", email: "", message: "" });
                setErrors({});
            } else {
                toast.error(res.error || "Xəta baş verdi. Yenidən cəhd edin.", { id: toastId });
            }
        } catch {
            toast.error("Xəta baş verdi. Zəhmət olmasa yenidən cəhd edin.", { id: toastId });
        } finally {
            setSubmitting(false);
        }
    };
                        ...
                        <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-5">
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="text" value={formData.name} onChange={set("name")} placeholder="Adınız" disabled={submitting} className={`${inputBase} ${errors.name ? inputErr : inputOk}`} />
                                ...
                                <div>
                                    ...
                                    <input type="text" value={formData.surname} onChange={set("surname")} placeholder="Soyadınız" disabled={submitting} className={`${inputBase} ${errors.surname ? inputErr : inputOk}`} />
                            ...
                            <div>
                                ...
                                <input type="tel" value={formData.phone} onChange={set("phone")} placeholder="010 XXX XX XX" disabled={submitting} className={`${inputBase} ${errors.phone ? inputErr : inputOk}`} />
                            ...
                            <div>
                                ...
                                <input type="email" value={formData.email} onChange={set("email")} placeholder="email@example.com" disabled={submitting} className={`${inputBase} ${errors.email ? inputErr : inputOk}`} />
                            ...
                            <div>
                                ...
                                <textarea value={formData.message} onChange={set("message")} rows={4} placeholder="Sualınız, maraq duyduğunuz proqram..." disabled={submitting} className={`${inputBase} resize-none ${errors.message ? inputErr : inputOk}`} />
                            ...
                            <button
                                type="submit"
                                disabled={submitting}
                                className="w-full py-4 bg-primary text-slate-900 rounded-xl font-bold text-sm hover:brightness-105 hover:-translate-y-0.5 transition-all flex items-center justify-center gap-2 disabled:opacity-60 disabled:cursor-not-allowed disabled:translate-y-0"
                            >
                                {submitting ? (
                                    <>
